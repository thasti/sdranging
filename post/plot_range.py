...
ax1.set_title("Es'Hail 2 ranging results - mean error: %.02f km" % np.mean(error))
ax1.plot(range_ts, range_km_tle, color='black', label="Prediction")
ax1.plot(range_ts, range_km_meas, '.', color='black', label="Measurement")
ax1.grid()
ax1.legend()
ax1.set_ylabel("Slant range [km]")
ax1.set_ylim((np.min(range_km_tle)-5, +np.max(range_km_tle)+10))
ax1.set_xlim((min(range_ts), max(range_ts)))

ax2.plot(range_ts, error, '.', color='black', label="Residuals")
ax2.set_xlabel("Time")
ax2.set_ylabel("Residuals [km]")
ax2.grid()
ax2.legend()
